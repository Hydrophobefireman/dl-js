...
class URL:
        ...
        **kwargs,
        ...
        if not hasattr(req, method):
            raise AttributeError(
                f"Requests library does not support method {method}"
            )
        res = getattr(self.session, method)(
            str(self),
            allow_redirects=True,
            headers=basic_headers
            if not "headers" in kwargs else kwargs.pop("headers"),
            **kwargs,
        )
    ...
    def update_url_meta_data(self) -> None:
        ...
        try:
            ret = req.head(
                str(self), headers=basic_headers, allow_redirects=True
            )
    ...
    def get_suggested_filename(self, ) -> str:
        ...
        return self._set_extension(
            self.get_filesafe_url(
                (
                    qs.get("file") or qs.get("filename") or
                    qs.get("download") or token_urlsafe(10)
                )
            )
        )
    ...
    def file_extension(self):
        if not self.has_meta_data:
            return ".bin"
        ct = self._m_headers.get("content-type",
                                 "").lower().split(";")[0].strip()
